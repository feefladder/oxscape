        ...
        terminal.draw(|frame| {
            ...
            sim.order()
                ...
                .for_each(|(i, lvl)| {
                    for c in lvl {
                        ...
                        let tx = u16::try_from(x * 2).unwrap();
                        let ty = u16::try_from(y).unwrap();
                        let color = sim
                            .gradient()
                            .eval_rational(sim.order().n_levels() - i, sim.order().n_levels());
                        buf[(tx, ty)].set_fg(Rgb(color.r, color.g, color.b));
                        buf[(tx + 1, ty)].set_fg(Rgb(color.r, color.g, color.b));
                    }
                });
            for y in 0..sim.order().meta().height() {
                for x in 0..sim.order().meta().width() {
                    ...
                    let mut n_dirs = 0;
                    for (dir, val) in sim.order().flows()[n].iter().enumerate() {
                        if *val != NO_FLOW_GEN {
